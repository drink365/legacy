# app.py — 《影響力》傳承策略平台｜TED風首頁＋登入＋到期顯示
import streamlit as st
from datetime import datetime
from utils.branding import APP_TITLE, APP_KICKER, APP_TAGLINE, FOOTER, LOGO_PATH, HIDE_DEFAULT_UI
...
if st.session_state.user:
    st.markdown(topbar_html(st.session_state.user), unsafe_allow_html=True)
...
with right:
    ...
    st.markdown(
        f'<div class="hero">'
        f'<h1>讓家族資產一目了然，<br/>讓顧問提案快速成交。</h1>'
        f'<p class="lead">{APP_TAGLINE}</p>'
        f'</div>',
        unsafe_allow_html=True
    )
    st.markdown(
        '<span class="pill">高資產家庭</span>'
        '<span class="pill">創辦人與二代</span>'
        '<span class="pill">保險與財稅顧問</span>',
        unsafe_allow_html=True
    )
...
with cta1:
    ...
    st.page_link("pages/1_insurance_planner.py", label="開始體驗", icon="👉")
with cta2:
    st.markdown("**📊 遺產稅試算與壓縮**")
    st.markdown('<p class="muted">先看稅，再排解；用保單與節點設計壓縮負擔。</p>', unsafe_allow_html=True)
    st.page_link("pages/2_estate_tax.py", label="立即試算", icon="👉")
with cta3:
    st.markdown("**🧑‍💼 顧問專用方案**")
    st.markdown('<p class="muted">一鍵生成提案＋對話語庫，提升成交率。</p>', unsafe_allow_html=True)
    st.page_link("pages/3_advisor_plan.py", label="了解方案", icon="👉")

...
sale_building_value = st.number_input("未來房屋評定現值（萬元）", value=240)
...
if is_self_use == "是" and hold_years >= 6:
    ...
    land_formula = f"{increment_amount:.1f} × 10%"
    land_note += "，自用住宅，適用10%稅率"
else:
    if increment_amount <= 400:
        land_tax = increment_amount * 0.20
        land_formula = f"{increment_amount:.1f} × 20%"
    elif increment_amount <= 800:
        land_tax = 400 * 0.20 + (increment_amount - 400) * 0.30
        land_formula = f"400 × 20% + ({increment_amount:.1f} - 400) × 30%"
    else:
        land_tax = 400 * 0.20 + 400 * 0.30 + (increment_amount - 800) * 0.40
        land_formula = f"400 × 20% + 400 × 30% + ({increment_amount:.1f} - 800) × 40%"
    land_note += "，一般用地累進稅率20~40%"

# 契稅、贈與稅、遺產稅、印花稅
gift_tax = 0
inherit_tax = 0
stamp_tax = sale_price * 0.001  # 印花稅 0.1%
stamp_formula = f"{sale_price:.1f} × 0.1%"
deed_tax = sale_price * 0.06    # 契稅 6%
deed_formula = f"{sale_price:.1f} × 6%"
...
if registration_target == "父母" and transfer_plan == "未來贈與給子女":
    ...
    if taxable_amount <= 256:
        ...
        gift_formula = f"{taxable_amount:.1f} × 10%"
    elif taxable_amount <= 512:
        gift_tax = 256 * 0.10 + (taxable_amount - 256) * 0.15
        gift_formula = f"256 × 10% + ({taxable_amount:.1f} - 256) × 15%"
    else:
        gift_tax = 256 * 0.10 + 256 * 0.15 + (taxable_amount - 512) * 0.20
        gift_formula = f"256 × 10% + 256 × 15% + ({taxable_amount:.1f} - 512) × 20%"
    ...
    if taxable_amount <= 5000:
        ...
        inherit_formula = f"{taxable_amount:.1f} × 10%"
    elif taxable_amount <= 10000:
        inherit_tax = 5000 * 0.10 + (taxable_amount - 5000) * 0.15
        inherit_formula = f"5000 × 10% + ({taxable_amount:.1f} - 5000) × 15%"
    else:
        inherit_tax = 5000 * 0.10 + 5000 * 0.15 + (taxable_amount - 10000) * 0.20
        inherit_formula = f"5000 × 10% + 5000 × 15% + ({taxable_amount:.1f} - 10000) × 20%"
...
if hold_years < 2:
    ...
    income_formula = f"{capital_gain:.1f} × 45%"
elif hold_years < 5:
    income_tax = capital_gain * 0.35
    income_formula = f"{capital_gain:.1f} × 35%"
elif hold_years < 10:
    income_tax = capital_gain * 0.20
    income_formula = f"{capital_gain:.1f} × 20%"
else:
    income_tax = capital_gain * 0.15
    income_formula = f"{capital_gain:.1f} × 15%"

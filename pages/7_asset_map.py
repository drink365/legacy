...
with st.form("asset_form"):
    ...
    with col1:
        equity = st.number_input("公司股權 (萬元)", min_value=0, value=st.session_state.asset_data["公司股權"], step=100)
        real_estate = st.number_input("不動產 (萬元)", min_value=0, value=st.session_state.asset_data["不動產"], step=100)
        financial = st.number_input("金融資產（存款、股票、基金等）(萬元)", min_value=0, value=st.session_state.asset_data["金融資產"], step=100)
    with col2:
        insurance = st.number_input("保單 (萬元)", min_value=0, value=st.session_state.asset_data["保單"], step=100)
        overseas = st.number_input("海外資產 (萬元)", min_value=0, value=st.session_state.asset_data["海外資產"], step=100)
        others = st.number_input("其他資產 (萬元)", min_value=0, value=st.session_state.asset_data["其他資產"], step=100)
...
- 若有其他資產：建議進一步盤點其性質與稅務歸屬，評估適合的傳承工具。
""")
